...
bool crescList(TElement c1, TElement c2) {
    if (c1 <= c2) {
        return true;
    } else {
        return false;
...
bool descList(TElement c1, TElement c2) {
    if (c1 >= c2) {
        return true;
    } else {
        return false;
...
vector<int> randomList(int cMin, int cMax, int s, int m) {
    vector<int> v;
...
vector<int> randomList(int cMin, int cMax) {
    vector<int> v;
    for (int c = cMin; c <= cMax; c++) {
        v.push_back(c);
    }
...
//2) valorile v, v>=cMin si v<=cMax care sunt multiplu de m (dar nu de s) sunt continute de c/m + 1 ori
//3) valorile >=cMin si <=cMax apar in ordine aleatoare
int populeazaList(LO &lo, int cMin, int cMax, int s, int m) {
    vector<int> v = randomList(cMin, cMax, s, m);
    int n = v.size();
    for (int i = 0; i < n; i++) {
        lo.adauga(v[i]);
    }
...
void populeazaList(LO &lo, int cMin, int cMax) {
    vector<int> v = randomList(cMin, cMax);
    int n = v.size();
    for (int i = 0; i < n; i++) {
        lo.adauga(v[i]);
    }
...
void testAdaugaSiCautaList(Relatie r) {
    LO lo = LO(r);
    int vMin = 10;
    int vMax = 30;
    int s = 5;
    int m = 3;
    int n = populeazaList(lo, vMin, vMax, s, m);
    assert(!lo.vida());
    assert(lo.dim() == n);
...
void testStergeCautaList(Relatie r) {
    LO lo = LO(r);
    int vMin = 0;
    int vMax = 100;
    populeazaList(lo, vMin, vMax);
    ...
    vector<int> vs = randomList(vMin, vMax);
    int n = vs.size();
    for (int i = 0; i < n; i++) {
        int v = vs[i];
        try {
...
void testStergeCautaList() {
    testStergeCautaList(crescList);
    testStergeCautaList(descList);
}

void testAdaugaSiCautaList() {
    testAdaugaSiCautaList(crescList);
    testAdaugaSiCautaList(descList);
}


void testCantitativList() {
    LO lo = LO(crescList);

    int vMin = 3000;
    int vMax = 6000;
    vector<int> valori = randomList(vMin, vMax);
    int n = valori.size();
    for (int i = 0; i < n; i++) {
        lo.adauga(valori[i]);
    }
...
void testAllExtinsList() {
    testCreeazaList();
    testAdaugaSiCautaList();
    testStergeCautaList();
    testCantitativList();
}
