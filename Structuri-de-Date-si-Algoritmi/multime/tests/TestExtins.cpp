...
void testCreeaza() {
    ...
    IteratorMultime im = m.iterator();  // iterator pe multimea vida ar trebui
...
void testAdauga() {
    ...
    assert((vverif[5] == 5) && (vverif[4] == 4) && (vverif[3] == 3) &&
           (vverif[2] == 2) && (vverif[1] == 1) && (vverif[0] == 0));
...
void testIterator() {
    ...
    IteratorMultime im2 =
        m.iterator();  // daca iterez, doar 33 poate sa-mi dea iteratorul
    ...
    Multime m2;
    for (int i = -100; i < 100;
         i++) {  // adaug 200 de elemente, fiecare de 3 ori
        m2.adauga(i);
        m2.adauga(i);
        m2.adauga(i);
    }
    ...
    int count = 0;
    while (im4.valid()) {  // fiecare element e multiplu de 4 si sunt in total
                           // 50 de elemente
        TElem e = im4.element();
        assert(e % 4 == 0);
        im4.urmator();
...
void testQuantity() {  // scopul e sa adaugam multe date
    ...
    im.prim();
    while (im.valid()) {  // fiecare element returnat de iterator trebuie sa fie
                          // in multime
        TElem e = im.element();
...
// nu stim reprezentarea multimii, putem testa doar anumite lucruri generale, nu
// stim in ce ordine vor fi afisate elementele.
void testAllExtins() {
    testCreeaza();
    testAdauga();
    testSterge();
