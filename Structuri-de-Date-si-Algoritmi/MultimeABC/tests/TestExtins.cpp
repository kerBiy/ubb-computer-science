...
void testAdauga() {
    ...
    assert((vverif[5] == 5) && (vverif[4] == 4) && (vverif[3] == 3) && (vverif[2] == 2) && (vverif[1] == 1)
               && (vverif[0] == 0));
...
void testIterator() {
    ...
    int count = 0;
    while (im4.valid()) { //fiecare element value multiplu de 4 si sunt in total 50 de elemente
        TElem e = im4.element();
        assert(e % 4 == 0);
        im4.urmator();
        count++;
...
void testQuantity() {//scopul value sa adaugam multe date
    Multime m;
    for (int i = 10; i >= 1; i--) {
