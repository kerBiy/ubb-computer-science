from business.manager import (
    manager_eliminate_all_expenses_of_type,
...
def ui_filter_command(expenses: list[list], cmd: list[str]) -> None:
    ...
    options = {
        "-t": ui_eliminate_all_expenses_of_type,
...
def ui_eliminate_all_expenses_of_type(expenses: list[list], cmd: list[str]) -> None:
    if len(cmd) != 1:
        raise Exception("Invalid command parameters.")

    type = cmd[0]

    manager_eliminate_all_expenses_of_type(expenses, type)
