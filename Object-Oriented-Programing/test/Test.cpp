...
void Test::testService() {
    ...
    try {
        service.addTask(9, description1, {}, status1);
    } catch (const std::exception &e) {}
    ...
    auto res4 = service.getTasksByStatus(status1);
    assert(res4.size() == 1);

    service.setTaskStatus(id1, status2);

    try {
        service.setTaskStatus(91, "closed");
    } catch (const std::exception &e) {}

    try {
        service.setTaskStatus(id1, "");
    } catch (const std::exception &e) {}
